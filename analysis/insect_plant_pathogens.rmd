...
```{r}
pathogen_insect <- condensed_spp %>% inner_join(insects_pathogens, by = join_by("species"=="targetTaxonName")) %>%  
  select(c(species, interactionTypeName, sourceTaxonName))
write_csv(pathogen_insect, "insect_interactions_pathogen_host.csv")
```

https://ropensci.org/blog/2020/08/25/scientific-name-parsing/

Extracting the plants pathogen hosts. 
```{r}
img_isolation_site <- plant_pathogens$`IMG ISOLATION SITE`
plants_pathogens_host <- data.frame(ID=character(), plant_host=character())

make_one_two_combinations <- function(string){
  one_two_comb <- c()
  reference_vec <- strsplit(string, " ")[[1]]
  for (idx in 1:length(reference_vec)-1){
    two_comb <-
    paste0(paste(c(reference_vec[idx], reference_vec[idx+1]), collapse=" "))
    one_two_comb <- append(one_two_comb, c(reference_vec[idx], two_comb))
  }
  
  return(unique(one_two_comb))
}
get_all_combinations <- function(strs_vecs){
  all_combinations <- c()
  for (strs_vec in strs_vecs){
    all_combinations <- append(all_combinations, make_one_two_combinations(strs_vec))
  }
  return(unique(all_combinations))
}

for (idx in 1:length(img_isolation_site)){
  isolation_site <- img_isolation_site[[idx]]
  
  el <- tibble::tibble(plants_name =
                         taxadb::clean_names(get_all_combinations(strsplit(img_isolation_site[[idx]], "\\|")[[1]])))
  name2taxid_plants <- taxizedb::name2taxid(el$plants_name, out_type = 'summary')
  if(nrow(name2taxid_plants) > 0){
    taxized_db_plants <- unique(taxizedb::taxid2name(name2taxid_plants$id))
    plants_hosts <- tibble::tibble(
      ID = replicate(length(taxized_db_plants), plant_pathogens$ID[[idx]]), 
      plant_host = taxized_db_plants )
    
  } else {
    plants_hosts <- tibble::tibble(
      ID = c(plant_pathogens$ID[[idx]]), 
      plant_host = c(NA))
  }
  
  plants_pathogens_host <- plants_pathogens_host %>% rbind(plants_hosts)
}
```

```{r}
plant_pathogens <- plant_pathogens %>% left_join(plants_pathogens_host %>% filter(plant_host != "root"), by = "ID")
write_csv(plant_pathogens %>% select(c(ID, SPECIES, plant_host)), "extracted_plant_host.csv")
```
