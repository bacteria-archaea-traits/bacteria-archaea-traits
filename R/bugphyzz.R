...
gram_stain_maps <- c("gram stain negative"="negative", "gram stain positive"="positive", 
...
bugphyzz_to_condensed_species_mapping =tibble::tibble(
  bugphyzz = c("biosafety level", "aerophilicity", "gram stain", "motility",
               "growth temperature", "optimal ph", "coding genes", "genome size", "halophily"
               ),
  condensed_species = c("cogem_classification", "metabolism", "gram_stain", "motility",
                        "growth_tmp", "optimum_ph", "coding_genes", "genome_size", "range_salinity"
                        )
)

fill_missing_attribute_with_bugphyzz <- function(trait, condensed_spp, trait_mappers=NULL){
  
  condensed_spp_keyword_mapping <- .keyword_cond_spp_mapping(bugphyzz_to_condensed_species_mapping, 
                                                             trait)
  cat("Get physiology data from bugphyzz")
  bugphyzz_attribute <- .get_cached_physiologies(trait)
  cat("Getting taxonomy ids")
  taxonomized_physiology_bugphyzz <- .get_tax_id(bugphyzz_attribute)
  cat("Merging data to species level; using data source priority based on bugphyzz definitions")
  species_level_physiology <- .bugphyzz_merger(taxonomized_physiology_bugphyzz, trait)
  ...
  cat("Get species with missing %s.", condensed_spp_keyword_mapping)
  ...
  filled_condensed_traits <- .fill_missing_condensed_spp_bugphyzz(
    ...
    species_level_physiology) %>% filter(!is.na(!!as.symbol(condensed_spp_keyword_mapping)))
  
  ## join back to condensed_spp
  cat(sprintf("Missing values for %s are : %s, filled %s ==> filling percentage %s", 
              trait, 
  ...
  left_suffix= ".x"
  right_suffix= ".y"
  left_column_name <- paste(condensed_spp_keyword_mapping, left_suffix, sep="")
  right_column_name <- paste(condensed_spp_keyword_mapping, right_suffix, sep="")
  data <-
    condensed_spp %>% left_join(filled_condensed_traits, by = "species_tax_id") %>%
      mutate(!!as.symbol(condensed_spp_keyword_mapping) :=
             ifelse(is.na(!!as.symbol(left_column_name)),
                    !!as.symbol(right_column_name),
                    !!as.symbol(left_column_name)
                    )) %>% 
      select(-c(!!as.symbol(right_column_name), !!as.symbol(left_column_name)))
...
.mappers <- function(data, mapping_dictionary,condensed_spp_keyword_mapping) {
...
.get_tax_id <- function(physiology_data) {
  ...
  cat(sprintf(
    "Missing taxonomy NCBIs: %s, \n and %s resolved with taxizedb",
    nrow(missing_tax),
  ...
  physiology_data <- rbind(physiology_data, fill_by_taxon_name) %>%
    mutate(tax_id = as.double(tax_id)) %>%
    filter(!is.na(tax_id)) %>% filter(Rank != "genus") %>%
    inner_join(tax %>% select(c(species_tax_id, tax_id)), by = "tax_id") %>% select(-c(tax_id))
  return(physiology_data)
}
...
.set_bugphyzz_data_source_priority_mergers <- function(){
  ...
  bugphyzz_source_path <- system.file("extdata", "attribute_sources.tsv", 
                                      package="bugphyzz")
  attribute_sources <- readr::read_tsv(bugphyzz_source_path) %>% 
    select(-c(full_source))
  ### Defining the tables priorities
  
  evidence_priority <- tibble::tibble(
    evidence = c("exp", "igc", "tas", "nas"), 
    priority = c(4, 3, 2, 1))
  confidence_priority <- tibble::tibble(
  ...
  attribute_sources <- attribute_sources %>%
    ...
    left_join(confidence_priority, by = c("Confidence_in_curation" = "confidence")) %>%
    rename(confidence_priority = priority)
  
  attribute_sources <- attribute_sources %>% 
    mutate(data_source_priority = confidence_priority * evidence_priority)
  write_csv(attribute_sources, "output/prepared_references/bugphyzz_data_source_priority.csv")
}
...
.get_priority <- function(){
  ...
  return(readr::read_csv("output/prepared_references/bugphyzz_data_source_priority.csv"))
}
...
.keyword_cond_spp_mapping<- function(bugphyzz_to_condensed_species_mapping, keyword){
