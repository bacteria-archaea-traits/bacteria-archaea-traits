...
#combined_cogem_pdf_data(cogem_data_pdf, data_columns = data_columns)
...
.merger <- function(data, tax){
  ...
  tax_cogem_data <- inner_join(data, tax, by = by, relationship="one-to-one") %>% 
                          rename(species=species.y) %>% select(-c(species.x, spec))
  return(tax_cogem_data)
}
...
#cogem_file_path <- "output/prepared_data/cogem_classification.csv"
